...
churro = "postgresql://postgres:postgresql@34.34.169.17/postgres"
...
def predict():
    ...
    cylinders = request.args.get("cylinders", None) #Acordarse del .get
    ...
    data = [cylinders, displacement, horsepower, weight, acceleration, model_year, origin]

    if None in data:
        return str(-999)
    else:
        predict_df = pd.DataFrame(np.array([data]).reshape(1,-1), columns=saved_model.feature_names_in_)

        date = str(datetime.now())[0:19]
        inputs = str(data)
        outputs = str(saved_model.predict(predict_df)[0])
